...
func TestIntegration_UriageJisha_CRUD(t *testing.T) {
	pool := setupTestDB(t)
	defer pool.Close()

	repo := NewUriageJishaRepository(pool)
	...
	t.Run("Create", func(t *testing.T) {
		...
		t.Run("Delete", func(t *testing.T) {
			...
			if err != ErrUriageJishaNotFound {
				t.Errorf("Delete: GetByPrimaryKey should return ErrUriageJishaNotFound, got %v", err)
			}
