...
func TestIntegration_CarInsSheetIchibanCars_CRUD(t *testing.T) {
	pool := setupTestDB(t)
	defer pool.Close()

	repo := NewCarInsSheetIchibanCarsRepository(pool)
	...
	t.Run("Create", func(t *testing.T) {
		record, err := repo.Create(ctx, orgID, electCertMgNo, electCertPublishdateE, electCertPublishdateY, electCertPublishdateM, electCertPublishdateD, &idCars)
		if err != nil {
		...
		t.Run("GetByPrimaryKey", func(t *testing.T) {
			fetched, err := repo.GetByPrimaryKey(ctx, orgID, electCertMgNo, electCertPublishdateE, electCertPublishdateY, electCertPublishdateM, electCertPublishdateD)
			if err != nil {
		...
		t.Run("Update", func(t *testing.T) {
			...
			updated, err := repo.Update(ctx, orgID, electCertMgNo, electCertPublishdateE, electCertPublishdateY, electCertPublishdateM, electCertPublishdateD, &newIdCars)
			if err != nil {
		...
		t.Run("Delete", func(t *testing.T) {
			err := repo.Delete(ctx, orgID, electCertMgNo, electCertPublishdateE, electCertPublishdateY, electCertPublishdateM, electCertPublishdateD)
			if err != nil {
				t.Fatalf("Delete failed: %v", err)
			}

			// Verify deletion
			_, err = repo.GetByPrimaryKey(ctx, orgID, electCertMgNo, electCertPublishdateE, electCertPublishdateY, electCertPublishdateM, electCertPublishdateD)
			if err != ErrCarInsSheetIchibanCarsNotFound {
				t.Errorf("Delete: GetByPrimaryKey should return ErrCarInsSheetIchibanCarsNotFound, got %v", err)
			}
