...
func TestIntegration_CarInspectionFilesA_CRUD(t *testing.T) {
	pool := setupTestDB(t)
	defer pool.Close()

	repo := NewCarInspectionFilesARepository(pool)
	ctx := context.Background()

	// Create unique test data
	orgID := fmt.Sprintf("test-org-%s", uuid.New().String()[:8])
	now := time.Now().Format(time.RFC3339)
	testData := &CarInspectionFilesA{
	...
	t.Run("Create", func(t *testing.T) {
		...
		t.Run("Delete", func(t *testing.T) {
			...
			if err != ErrCarInspectionFilesANotFound {
				t.Errorf("Delete: GetByUUID should return ErrCarInspectionFilesANotFound, got %v", err)
			}
